...
.wa-float-btn:hover { transform: scale(1.08); box-shadow: 0 12px 36px rgba(37,211,102,0.55); }
...
.wa-float-popup.open { display: block; }
/* Hide popup on mobile — button goes direct to WA */
@media (max-width: 767px) {
    .wa-float-popup { display: none !important; }
}
...
.wa-popup-header-text strong { display: block; font-size: 0.88rem; font-weight: 800; color: #1a1a2e; }
.wa-popup-header-text span { font-size: 0.75rem; color: #22c55e; font-weight: 600; }
.wa-popup-body {
    background: #f8f4ff;
    border-radius: 12px;
    padding: 12px 14px;
...
.wa-popup-body strong { color: #1a1a2e; }
.wa-popup-cta {
    display: flex;
    align-items: center;
    justify-content: center;
...
.wa-popup-cta:hover { background: #1da851; transform: scale(1.02); }
.wa-float-btn { position: relative; }
.lp-hero-trust {
    display: flex;
    justify-content: center;
    gap: 20px;
...
<div class="wa-float-wrap" id="waFloatWrap">
    <!-- Popup card -->
    <div class="wa-float-popup" id="waFloatPopup">
        <div class="wa-popup-header">
            <div class="wa-popup-avatar"><i class="fas fa-headset"></i></div>
            <div class="wa-popup-header-text">
                <strong>dopagency – Studio Foto</strong>
                <span>● Online Sekarang</span>
            </div>
        </div>
        <div class="wa-popup-body">
            Halo! 👋 Tertarik punya <strong>Website + Booking System</strong> untuk studio foto Anda?<br>
            Klik tombol di bawah untuk konsultasi & pesan sekarang!
        </div>
        <a href="https://wa.me/6282125189383?text=Halo%20dopagency%2C%20saya%20mau%20pesan%20Website%20Studio%20Foto%20%2B%20Booking%20System%2C%20boleh%20konsultasi%3F" target="_blank" class="wa-popup-cta">
...
(function(){
    ...
    var popup = document.getElementById('waFloatPopup');
    var badge = btn.querySelector('.wa-badge');
    var waUrl = 'https://wa.me/6282125189383?text=Halo%20dopagency%2C%20saya%20mau%20pesan%20Website%20Studio%20Foto%20%2B%20Booking%20System%2C%20boleh%20konsultasi%3F';
    var isMobile = function(){ return window.innerWidth < 768; };

    // Auto open popup on desktop after 3s
    setTimeout(function(){
        if (!isMobile()) popup.classList.add('open');
    }, 3000);

    btn.addEventListener('click', function(){
        if (isMobile()) {
            // Direct to WhatsApp on mobile
